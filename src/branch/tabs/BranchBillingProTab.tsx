...
import {
  AlertTriangle, Banknote, CreditCard, Eraser, Gift, HelpCircle, IndianRupee,
  Keyboard, Package, PauseCircle, Printer, Receipt, RotateCcw, Search, Smartphone,
  Tag, Trash2, User, WalletCards, XCircle, Plus, Minus, ClipboardList,
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { useAuthStore } from '@/stores/authStore';
import { useBranchStore, type StockItem } from '../branchStore';
import { BRANCH_LABELS, type Branch } from '../types';
import { SNB_CATEGORIES, SNB_ITEMS, type SnbCategory, type SnbItem } from '../snbItems';
import { VRSNB_CATEGORIES, VRSNB_ITEMS, type VrsnbCategory, type VrsnbItem } from '../vrsnbItems';
import {
...
function stockFor(stock: StockItem[], itemName: string) {
  const found = stock.find((s) => s.itemName.toLowerCase() === itemName.toLowerCase());
...
function printBranchBill(bill: BranchBillRecord, duplicate = false) {
  ...
  const html = `<!doctype html><html><head><title>${title} ${bill.billNo}</title><style>
    ...
    <div class="row"><span>Salesperson</span><span>${bill.salesperson}</span></div><div class="row"><span>Biller</span><span>${bill.biller}</span></div><div class="dash"></div>
    ...
    <div class="row"><span>Tendered</span><span>₹${bill.tendered.toFixed(2)}</span></div><div class="row"><span>Balance</span><span>₹${bill.balance.toFixed(2)}</span></div><div class="row"><span>Mode</span><span>${bill.paymentMode.toUpperCase()}</span></div>
...
export default function BranchBillingProTab({ branch, branchStock, onOpenTab }: Props) {
  ...
  }, [branch, salespeople, userName]);

  const stockMap = useMemo(() => {
    const map = new Map<string, number>();
    branchStock.forEach((s) => map.set(s.itemName.toLowerCase(), Number(s.quantity || 0)));
    return map;
  }, [branchStock]);
  ...
  const addItem = useCallback((item: BillingItem, amount = unitOf(item) === 'kg' ? 0.25 : 1) => {
    ...
    const available = stockMap.get(item.name.toLowerCase()) ?? 0;
    if (available <= 0) {
    ...
    setCart((current) => {
      ...
      if (existing) return current.map((c) => c.itemName === item.name ? { ...c, quantity: newQty, lineTotal: newQty * c.price } : c);
  ...
  const reduceItem = (itemName: string) => setCart((current) => current.flatMap((c) => {
    ...
    return next > 0 ? [{ ...c, quantity: next, lineTotal: next * c.price }] : [];
  }));

  const clear = useCallback(() => {
    setCart([]); setCashTendered(''); setSplit({ cash: '', upi: '', card: '' }); setDiscount(''); setError(''); setLastBill(null);
  }, []);

  const holdBill = useCallback(() => {
    if (!cart.length) { setError('Cart is empty.'); return; }
    if (!salesperson) { setError('Select salesperson before holding bill.'); return; }
    addHold({ branch, salesperson, items: cart, note: `Held by ${userName}` });
    clear();
    setShowHold(true);
  }, [cart, salesperson, branch, userName, addHold, clear]);
  ...
  const validateCheckout = () => {
    if (!salesperson) return 'Salesperson selection is mandatory before billing.';
    ...
    try {
      for (const item of cart) {
        const err = branch === 'SNB' || branch === 'Hosur' || branch === 'VRSNB'
          ? await recordSnbSale(branch, item.itemName, item.quantity, salesperson, modeLabel, item.price, billNo)
          : { error: await recordSale(branch, item.itemName, item.quantity, salesperson, modeLabel, billNo, item.price), mismatch: false };
        if (err.error) throw new Error(err.error);
      }
      const saved = addBill({
        branch, billNo, invoiceNo, items: cart, subtotal, discount: discountValue, tax, total,
        tendered: paymentMode === 'cash' || paymentMode === 'split' ? tendered : total,
        balance: paymentMode === 'cash' || paymentMode === 'split' ? balance : 0,
        paymentMode,
      ...
      setLastBill(saved);
      setCart([]); setCashTendered(''); setSplit({ cash: '', upi: '', card: '' }); setDiscount('');
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'F5') { e.preventDefault(); setPaymentMode('cash'); }
      if (e.key === 'F6') { e.preventDefault(); setPaymentMode('upi'); setCashTendered(String(total)); }
      if (e.key === 'F7') { e.preventDefault(); setPaymentMode('card'); setCashTendered(String(total)); }
      if (e.key === 'F8' || (e.ctrlKey && e.key.toLowerCase() === 'p')) { e.preventDefault(); checkout(); }
      if (e.key === 'F9') { e.preventDefault(); onOpenTab?.('advance'); }
      if (e.key === 'F10') { e.preventDefault(); onOpenTab?.('quotation'); }
  ...
  }, [addItem, branch, cart, clear, holdBill, onOpenTab, selectedIndex, subtotal, total, visibleItems]);

  return (
    <div className="branch-billmaxo h-full min-h-[calc(100dvh-var(--header-h,4rem)-7rem)] overflow-hidden rounded-[2rem] border border-slate-200 bg-slate-100 shadow-xl shadow-slate-200/70">
        ...
        <aside ref={cartRef} tabIndex={-1} className="flex min-h-0 flex-col border-r border-slate-200 bg-white focus:outline-none focus:ring-4 focus:ring-amber-300/30">
          <div className="border-b border-slate-200 bg-slate-950 p-5 text-white">
            <div className="flex items-center justify-between gap-3">
              ...
              <button onClick={() => setShowShortcuts(true)} className="rounded-2xl bg-white/10 p-3 text-white hover:bg-white/20"><Keyboard className="size-5" /></button>
            </div>
                  ...
                  {cart.map((i) => (
                    <div key={i.itemName} className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0"><p className="text-base font-black leading-tight text-slate-950">{i.itemName}</p><p className="mt-1 text-xs font-bold text-slate-500">{i.quantity} {i.unit} × {money(i.price)}</p></div>
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        ...
                        <span className="min-w-16 rounded-xl bg-slate-950 px-3 py-2 text-center text-base font-black text-white tabular-nums">{i.quantity}</span>
          ...
          <div className="border-t border-slate-200 bg-white p-4 shadow-[0_-12px_35px_rgba(15,23,42,.08)]">
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div className="rounded-2xl bg-emerald-50 p-3 text-emerald-800"><p className="text-xs font-bold">Balance</p><p className="text-3xl font-black tabular-nums">{money(balance)}</p></div>
            </div>
            <div className="mt-3 grid grid-cols-4 gap-2">
              {[['cash','Cash',Banknote],['upi','UPI',Smartphone],['card','Card',CreditCard],['split','Split',WalletCards]].map(([key,label,Icon]) => (
                <button key={String(key)} onClick={()=>{setPaymentMode(key as PayMode); if(key !== 'cash' && key !== 'split') setCashTendered(String(total));}} className={cn('rounded-2xl border-2 p-2 text-xs font-black', paymentMode === key ? 'border-slate-950 bg-slate-950 text-white' : 'border-slate-200 bg-white text-slate-600')}><Icon className="mx-auto mb-1 size-4"/>{String(label)}</button>
              ))}
            </div>
            {paymentMode === 'split' ? (
              <div className="mt-3 grid grid-cols-3 gap-2"><input value={split.cash} onChange={(e)=>setSplit({...split,cash:e.target.value})} placeholder="Cash" className="rounded-xl border p-2 font-bold"/><input value={split.upi} onChange={(e)=>setSplit({...split,upi:e.target.value})} placeholder="UPI" className="rounded-xl border p-2 font-bold"/><input value={split.card} onChange={(e)=>setSplit({...split,card:e.target.value})} placeholder="Card" className="rounded-xl border p-2 font-bold"/></div>
            ) : (
        ...
        <main className="flex min-h-0 flex-col bg-slate-100">
          <div className="border-b border-slate-200 bg-white p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div><p className="text-xs font-black uppercase tracking-[0.25em] text-slate-400">Billmaxo style · keyboard first</p><h1 className="text-3xl font-black tracking-tight text-slate-950">{BRANCH_LABELS[branch]} Fast Billing</h1></div>
          ...
          <div className="border-b border-slate-200 bg-white/80 p-3">
            <div className="flex gap-2 overflow-x-auto pb-1">
              {['All', ...categories].map((c, idx) => <button key={c} onClick={()=>setCategory(c)} className={cn('shrink-0 rounded-2xl px-4 py-3 text-sm font-black transition', category === c ? 'bg-slate-950 text-white shadow-lg' : 'bg-white text-slate-600 ring-1 ring-slate-200 hover:bg-slate-50')}><span className="mr-2 text-[10px] opacity-60">{idx === 0 ? 'A' : idx}</span>{c}</button>)}
            </div>
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto p-4">
              ...
              {visibleItems.map((item, idx) => {
                ...
                return (
                  <button key={item.barcode} disabled={disabled} onClick={()=>addItem(item)} className={cn('group min-h-36 rounded-3xl border-2 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-45', idx === selectedIndex ? 'border-amber-400 ring-4 ring-amber-100' : 'border-slate-200', inCart && 'border-emerald-400 bg-emerald-50')}>
                    <div className="flex items-start justify-between gap-3"><p className="line-clamp-2 text-xl font-black leading-tight text-slate-950">{item.name}</p><span className="rounded-xl bg-slate-100 px-2 py-1 text-xs font-black text-slate-500">{idx + 1}</span></div>
                    <div className="mt-4 flex items-end justify-between gap-3"><div><p className="text-2xl font-black text-emerald-700">{money(item.price)}<span className="text-xs text-slate-400">/{unitOf(item)}</span></p><p className={cn('mt-1 text-sm font-black', disabled ? 'text-red-600' : stock < 5 ? 'text-amber-600' : 'text-slate-500')}><Package className="mr-1 inline size-4"/>{disabled ? 'Out of stock' : `${stock} ${unitOf(item)} left`}</p></div>{inCart && <span className="rounded-2xl bg-emerald-600 px-3 py-2 text-sm font-black text-white">{inCart.quantity}</span>}</div>
                  </button>
                );
              })}
            </div>
      ...
      {showShortcuts && <Modal onClose={() => setShowShortcuts(false)} title="Keyboard shortcut help"><div className="grid grid-cols-1 gap-2 sm:grid-cols-2">{shortcuts.map(([k,v])=><div key={k} className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3"><kbd className="rounded-lg bg-slate-950 px-2 py-1 text-sm font-black text-white">{k}</kbd><span className="text-sm font-bold text-slate-700">{v}</span></div>)}</div></Modal>}
