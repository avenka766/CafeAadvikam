...
export function SnbPurchaseReturnPanel() {
  ...
  const submit = async () => {
    ...
    await Promise.all([loadBase(), useBranchStore.getState().fetchBranchData("SNB", false, ['stock'])]); // EGRESS FIX: purchase return only touches stock
  };
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {/* BUG FIX (audit 2026-08-30): hardcoded step="0.001" and no
                ...
                decimal points" gap found and fixed across the Planner
                dashboard's own equivalent tabs today. */}
            <Field label="Quantity"><input type="number" min="0.001" step={lineDraft.unit === 'pcs' ? 1 : 0.001} className={inputClass} value={lineDraft.quantity} onChange={(event) => setLineDraft({ ...lineDraft, quantity: sanitizeQtyForUnit(event.target.value, lineDraft.unit === 'pcs' ? 'pcs' : 'kg') })} /></Field>
...
export function SnbAdvanceOrdersPanel() {
  ...
  useEffect(() => { void fetchBranchData("SNB", false, ['stock', 'advance']); }, [fetchBranchData]); // EGRESS FIX: this panel only reads stock + advance orders
