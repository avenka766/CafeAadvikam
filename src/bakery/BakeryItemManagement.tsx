// src/bakery/BakeryItemManagement.tsx  ← NEW FILE
//
// Admin screen for managing the bakery_items table.
// Accessible at /bakery/items  (admin only — see App.tsx diff below).
//
// Features
//  • View ALL items (enabled + disabled) grouped by category
//  • Toggle enable / disable per item
//  • Add a new item (name, category, icon emoji)
//  • Inline edit name / icon / category
...
const CATEGORY_META: Record<string, { emoji: string; color: string; bg: string }> = {
  ...
  Cookies:   { emoji: '🍪', color: 'text-brown-700',  bg: 'bg-yellow-50 border-yellow-200' },
};
...
function EditSheet({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end" onClick={onClose}>
      <div
        className="w-full bg-background rounded-t-2xl border-t border-border p-5 space-y-4 max-h-[80vh] overflow-y-auto"
      ...
      >
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              maxLength={4}
              className="w-20 h-10 text-center text-xl rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/30"
            />
          </div>
        ...
        {error && (
          <p className="text-xs font-body text-destructive bg-destructive/10 px-3 py-2 rounded-lg">{error}</p>
        )}

        <div className="flex gap-2 pt-1">
          <button onClick={onClose}
            className="flex-1 h-10 rounded-xl border border-border text-sm font-body font-semibold text-muted-foreground">
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={saving || !dirty || !name.trim()}
            className="flex-1 h-10 rounded-xl cafe-gradient text-primary-foreground text-sm font-body font-bold disabled:opacity-50 flex items-center justify-center gap-1.5"
          >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-6" onClick={onClose}>
      ...
      >
        ...
        <div className="flex gap-2">
          <button onClick={onClose}
            className="flex-1 h-10 rounded-xl border border-border text-sm font-body font-semibold">
            Cancel
          </button>
          <button onClick={onConfirm}
            className="flex-1 h-10 rounded-xl bg-destructive text-destructive-foreground text-sm font-body font-bold active:scale-95 transition">
            Delete
          </button>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end" onClick={onClose}>
      ...
      >
        ...
        <div className="space-y-3">
          {/* Category first so icon auto-fills */}
          <div>
            <label className="text-[10px] font-body font-bold text-muted-foreground uppercase mb-1 block">Category</label>
            <select
              value={category}
...
export default function BakeryItemManagement() {
  ...
  return (
    <div className="min-h-screen bg-background pt-14 pb-24 px-4">
      {/* header */}
      <div className="pt-4 pb-3 flex items-start justify-between gap-2">
        <div>
          <h1 className="font-display text-2xl font-bold text-foreground flex items-center gap-2">
            <Settings2 className="size-6 text-primary" /> Bakery Items
          </h1>
          <p className="text-xs font-body text-muted-foreground mt-0.5">
            Manage the item list shown to order receivers
          </p>
        </div>
        <button
          onClick={() => setShowAdd(true)}
          className="mt-1 h-9 px-4 rounded-xl cafe-gradient text-primary-foreground text-sm font-body font-bold flex items-center gap-1.5 shrink-0 active:scale-95 transition"
        >
      ...
      <div className="flex gap-2 mb-4">
        ...
        <button
          ...
          className={cn(
            'h-10 px-3 rounded-xl border text-xs font-body font-semibold transition',
            showDisabled
      ...
      {!loading && (
        <div className="space-y-3">
          {BAKERY_CATEGORIES.map(cat => {
            ...
            if (catItems.length === 0 && search) return null; // hide empty cats during search
            return (
              <CategorySection
                key={cat}
                category={cat}
