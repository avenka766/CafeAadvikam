...
// StatCard and DisputesAndReturnsPanel are reused as-is (not duplicated) by
// the new combined tab in PlannerDashboard.tsx; this component itself keeps
// only what it always owned exclusively — the cashier cash-reconciliation
// workflow and its own Closure History — see the removed 'operations'/
// 'disputes' sections below.
export function StatCard({ label, value, helper, icon, tone = 'slate' }: {
...
export function DisputesAndReturnsPanel({ onExportDataChange }: {
  ...
  // transient list) but still contributes a table to the combined PDF.
  const disputesExportSpecs = useMemo((): { excel: ExcelSectionSpec; pdf: PdfSectionSpec } => ({
    excel: {
      prefix: 'Disputes & Returns',
      sheets: [
        { name: 'Pending Returns', fallback: 'No pending returns', rows: returns.map(r => ({
          'From Branch': r.source_branch, Item: r.item_name, Quantity: r.expected_quantity, Unit: r.unit,
        ...
        { name: 'Open Disputes', fallback: 'No open disputes', rows: disputes.map(d => ({
          ...
          Reason: d.dispute_reason || '', 'Disputed By': d.disputed_by || '',
          'Disputed At': d.disputed_at ? new Date(d.disputed_at).toLocaleString('en-IN') : '',
        })) },
      ],
    },
    pdf: {
      ...
      tables: [
        {
          ...
          headers: ['From Branch', 'Item', 'Qty', 'Unit', 'Requested By', 'Requested At'],
        ...
        },
        {
          ...
          headers: ['Branch', 'Item', 'Dispatched', 'Received', 'Unit', 'Reason', 'By'],
          colWidths: [45, 130, 55, 55, 35, 130, 60],
          rows: disputes.map(d => [d.branch, d.item_name.slice(0, 26), String(d.quantity), d.disputed_received_quantity != null ? String(d.disputed_received_quantity) : '-', d.unit, (d.dispute_reason || '-').slice(0, 26), (d.disputed_by || '-').slice(0, 12)]),
  ...
  const exportExcel = () => generateExcelReport({ filename: `disputes-and-returns-${new Date().toISOString().slice(0, 10)}.xlsx`, sections: [disputesExportSpecs.excel] });
...
// FEATURE (2026-09-15): pulled out of PackingDailyClosureTab's own body
// (unchanged logic, just parameterized on `orders`/`date` instead of reading
// them off local component state) so the new combined Planner tab's Packing
// & Dispatch section reads the exact same numbers as this tab's own Cashier
// Closure payload/export always have — one calculation, not two copies that
// could quietly drift apart.
export function usePackingDispatchSummary(orders: import('./types').BakeryOrder[], date: string) {
  ...
  return { dayDispatches, packedOrders, dispatchedOrderIds, pendingOrders, branchSummary, itemSummary, leftoverRows, dispatchedKg, dispatchedPcs, leftoverKg };
}
...
export default function PackingDailyClosureTab({ onCounterStatusChange }: { onCounterStatusChange?: (isOpen: boolean) => void }) {
  ...
  const [error, setError] = useState('');
  // FEATURE (2026-09-15): "Packing & Dispatch" and "Disputes & Returns" moved
  // out to the new combined Planner tab (PlannerDashboard.tsx) — this tab now
  // owns only what it always owned exclusively: the cash-critical cashier
  // open/close/reconcile workflow and its own Closure History. See
  // usePackingDispatchSummary above and the exported DisputesAndReturnsPanel.
  const [section, setSection] = useState<'cashier'|'history'>('cashier');
  ...
  return <section className="space-y-5">
    ...
    <div className="flex gap-2 overflow-x-auto pb-1"><button onClick={()=>setSection('cashier')} className={cn('inline-flex shrink-0 items-center gap-2 rounded-xl border px-4 py-2.5 text-xs font-black',section==='cashier'?'border-teal-700 bg-teal-700 text-white':'bg-card')}><WalletCards className="size-4"/>Cashier Closure</button><button onClick={()=>setSection('history')} className={cn('inline-flex shrink-0 items-center gap-2 rounded-xl border px-4 py-2.5 text-xs font-black',section==='history'?'border-teal-700 bg-teal-700 text-white':'bg-card')}><History className="size-4"/>Closure History</button></div>
