...
const CLOSURE_KEY_PREFIX = 'packing-daily-closure:';
...
export default function PackingDailyClosureTab() {
  ...
  const loadClosure = useCallback(async () => {
    ...
    const key = `${CLOSURE_KEY_PREFIX}${date}`;
    const [{ data, error: rowError }, { data: historyData, error: historyError }] = await Promise.all([
      ...
      supabase.from('app_state').select('key,value,updated_at').like('key', `${CLOSURE_KEY_PREFIX}%`).order('updated_at', { ascending: false }).limit(30),
    ]);
    ...
    const mapRow = (row: { key: string; value: unknown; updated_at?: string | null } | null): PackingClosureRow | null => {
      ...
      const rowDate = value.business_date || row.key.slice(CLOSURE_KEY_PREFIX.length);
      return {
        id: value.id || row.key,
  ...
  const startCounter = async () => {
    ...
    const { error: saveError } = await supabase.from('app_state').upsert({
      key: `${CLOSURE_KEY_PREFIX}${date}`,
      value: payload,
      updated_at: now,
    }, { onConflict: 'key' });
    setSaving(false);
    if (saveError) return setError(saveError.message);
    setMessage('Packing cashier counter opened. Billing remains available even when the counter is not opened.');
    await loadClosure();
  };
  ...
  const finalize = async () => {
    ...
    const now = new Date().toISOString();
    const payload: PackingClosureRow = {
      id: record?.id ?? `${CLOSURE_KEY_PREFIX}${date}`,
      business_date: date,
      status: 'finalized',
    ...
    setSaving(false);
    if (saveError) return setError(saveError.message);
    setMessage('Packing daily closure finalized and saved.');
    await loadClosure();
  };
  ...
  const exportExcel = () => {
    ...
    const summary = [{
      ...
      'Opening Cash': Number(openingCash || 0), 'Expected Cash': expectedCash, 'Counted Cash': counted, Difference: difference,
    }];
  ...
  return <section className="space-y-5">
    <div className="overflow-hidden rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-950 via-emerald-950 to-slate-900 text-white shadow-xl">
      <div className="flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div><div className="mb-2 flex flex-wrap items-center gap-2"><span className="rounded-full bg-emerald-400/15 px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-emerald-200">Packing control</span><span className="rounded-full bg-white/10 px-3 py-1 text-[10px] font-black text-white/75">Billing works without opening counter</span></div><h2 className="font-display text-3xl font-black">Daily Closure & Cashier Handover</h2><p className="mt-1 max-w-3xl text-sm text-white/60">Finalize packing output, branch dispatch, leftovers, billing collections, credit, physical cash and staff handover in one closure.</p></div>
    ...
    {section === 'cashier' && <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><StatCard label="Bills" value={billCount} helper="Unique bill numbers" icon={<ClipboardCheck className="size-5"/>} tone="blue"/><StatCard label="Gross Sales" value={money(grossSales)} helper="Paid sales rows" icon={<IndianRupee className="size-5"/>} tone="emerald"/><StatCard label="Credit Billed" value={money(creditBilled)} helper="New credit balance" icon={<CreditCard className="size-5"/>} tone="amber"/><StatCard label="Credit Collected" value={money(creditCollected)} helper="Later collections" icon={<WalletCards className="size-5"/>} tone="blue"/><StatCard label="Expected Cash" value={money(expectedCash)} helper="Opening + cash collection" icon={<Banknote className="size-5"/>} tone="slate"/></div><div className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]"><div className="rounded-2xl border bg-card p-4 space-y-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-display text-lg font-black">Cashier Handover</h3><p className="text-xs text-muted-foreground">Opening is optional. Final closure requires physical cash count.</p></div><span className={cn('rounded-full px-3 py-1 text-xs font-black',finalized?'bg-slate-100 text-slate-700':record?.status==='draft'?'bg-emerald-100 text-emerald-700':'bg-amber-100 text-amber-700')}>{finalized?'Finalized':record?.status==='draft'?'Counter Open':'Not Open — billing allowed'}</span></div><div className="grid gap-3 sm:grid-cols-2"><label className="space-y-1"><span className="text-xs font-black text-muted-foreground">Opening Cash</span><input type="number" min="0" value={openingCash} disabled={finalized||record?.status==='draft'} onChange={e=>setOpeningCash(e.target.value)} className="h-11 w-full rounded-xl border bg-background px-3 text-sm font-bold"/></label><label className="space-y-1"><span className="text-xs font-black text-muted-foreground">Counted Closing Cash</span><input type="number" min="0" value={countedCash} disabled={finalized} onChange={e=>setCountedCash(e.target.value)} className="h-11 w-full rounded-xl border bg-background px-3 text-sm font-bold" placeholder="Physical cash count"/></label><label className="space-y-1 sm:col-span-2"><span className="text-xs font-black text-muted-foreground">Handover Notes</span><textarea value={notes} disabled={finalized} onChange={e=>setNotes(e.target.value)} className="min-h-24 w-full rounded-xl border bg-background p-3 text-sm" placeholder="Pending orders, cash variance, leftovers or branch handover notes"/></label></div><div className="flex flex-wrap gap-2"><button onClick={startCounter} disabled={saving||finalized||record?.status==='draft'} className="inline-flex h-11 items-center gap-2 rounded-xl border bg-card px-4 text-xs font-black disabled:opacity-50">{saving?<Loader2 className="size-4 animate-spin"/>:<CheckCircle2 className="size-4"/>}Open Counter (Optional)</button><button onClick={finalize} disabled={saving||finalized} className="inline-flex h-11 items-center gap-2 rounded-xl bg-emerald-700 px-4 text-xs font-black text-white disabled:opacity-50">{saving?<Loader2 className="size-4 animate-spin"/>:<ShieldCheck className="size-4"/>}Finalize Daily Closure</button></div></div><div className="space-y-4"><div className="rounded-2xl border bg-card p-4"><h3 className="font-black">Payment Breakdown</h3><div className="mt-3 space-y-2">{[['Cash',cashTotal],['UPI',paymentTotal('upi')],['Card',paymentTotal('card')],['Bank',paymentTotal('bank')],['Mixed',paymentTotal('mixed')]].map(([label,value])=><div key={String(label)} className="flex items-center justify-between rounded-xl bg-muted/35 px-3 py-2 text-sm"><span>{label}</span><b>{money(Number(value))}</b></div>)}</div></div><div className="rounded-2xl border bg-card p-4"><h3 className="font-black">Cash Reconciliation</h3><div className="mt-3 space-y-2">{[['Opening Cash',Number(openingCash||0)],['Cash Collection',cashTotal],['Expected Cash',expectedCash],['Counted Cash',counted],['Difference',difference]].map(([label,value])=><div key={String(label)} className="flex items-center justify-between rounded-xl bg-muted/35 px-3 py-2 text-sm"><span>{label}</span><b className={String(label)==='Difference'&&Number(value)!==0?'text-red-600':''}>{money(Number(value))}</b></div>)}</div></div></div></div></div>}
