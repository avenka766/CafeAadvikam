      ...
      <button className="w-full px-4 py-3.5 flex items-center gap-3 text-left active:bg-muted/20 transition-colors rounded-t-2xl"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className={cn(
              'rounded-full border px-2 py-0.5 text-[9px] font-body font-bold',
              order.targetBranch === 'VRSNB'
                ? 'border-blue-200 bg-blue-100 text-blue-700'
                : order.targetBranch === 'SNB'
                  ? 'border-amber-200 bg-amber-100 text-amber-700'
                  : order.targetBranch === 'Hosur'
                    ? 'border-emerald-200 bg-emerald-100 text-emerald-700'
                    : 'border-border bg-muted text-muted-foreground'
            )}>
              {order.targetBranch || 'Branch not set'}
            </span>
