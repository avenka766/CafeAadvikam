...
export default function AdminWalletTab() {
  ...
  const filtered = useMemo(() => wallets.filter((wallet) => {
    ...
    return queryMatch && (status === 'all' || wallet.status === status)
  ...
  }), [wallets, query, status, minimumBalance, maximumBalance, dateFrom, dateTo]);
  ...
  const submitEdit = async (event: React.FormEvent) => {
    ...
    try {
      const updated = await updateWalletCustomer(editTarget.id, { ...walletForm, reason: editReason });
  ...
  const exportRows = () => downloadCsv(`wallet-report-${today()}.csv`, filtered.map((wallet) => ({
    'Wallet No': wallet.walletNumber, Customer: wallet.customerName, Mobile: wallet.mobile, Status: wallet.status,
    'Paid Balance': wallet.paidBalance, 'Promotional Balance': wallet.promotionalBalance,
  ...
  const printWalletReport = () => {
    ...
    popup.document.write(`<html><head><title>Wallet Report</title><style>@page{size:auto;margin:6mm}@media print{html,body{height:auto !important}}body{font-family:Arial;padding:16px}table{width:100%;border-collapse:collapse;margin-top:18px}th,td{padding:8px;border:1px solid #ddd;text-align:left}h1{margin:0}small{color:#555}</style></head><body><h1>Customer Wallet Report</h1><small>${escapeHtml(new Date().toLocaleString('en-IN'))}</small><table><thead><tr><th>Wallet</th><th>Customer</th><th>Mobile</th><th>Status</th><th>Paid</th><th>Promo</th><th>Total</th></tr></thead><tbody>${filtered.map((wallet) => `<tr><td>${escapeHtml(wallet.walletNumber)}</td><td>${escapeHtml(wallet.customerName)}</td><td>${escapeHtml(wallet.mobile)}</td><td>${escapeHtml(wallet.status)}</td><td>${Math.round(wallet.paidBalance)}</td><td>${Math.round(wallet.promotionalBalance)}</td><td>${Math.round(wallet.totalBalance)}</td></tr>`).join('')}</tbody></table></body></html>`); popup.document.close(); popup.print();
  };

  const printStatement = (wallet: WalletCustomer) => {
    const rows = transactionsFor(wallet);
    const popup = window.open('', '_blank', 'width=1000,height=720'); if (!popup) return;
    popup.document.write(`<html><head><title>${escapeHtml(wallet.walletNumber)} Statement</title><style>@page{size:auto;margin:6mm}@media print{html,body{height:auto !important}}body{font-family:Arial;padding:16px}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:18px 0}.card{border:1px solid #ddd;padding:12px;border-radius:10px}table{width:100%;border-collapse:collapse}th,td{padding:7px;border:1px solid #ddd;text-align:left;font-size:12px}h1,h2,p{margin:0 0 6px}</style></head><body><h1>Cafe Aadvikam Wallet Statement</h1><p>${escapeHtml(wallet.customerName)} · ${escapeHtml(wallet.walletNumber)} · ${escapeHtml(wallet.mobile)}</p><p>Generated ${escapeHtml(new Date().toLocaleString('en-IN'))}</p><div class="grid"><div class="card"><b>Paid</b><br>₹${Math.round(wallet.paidBalance)}</div><div class="card"><b>Promotional</b><br>₹${Math.round(wallet.promotionalBalance)}</div><div class="card"><b>Available</b><br>₹${Math.round(wallet.totalBalance)}</div></div><table><thead><tr><th>Date</th><th>Type</th><th>Bill/Reference</th><th>Paid</th><th>Promo</th><th>Amount</th><th>Balance</th></tr></thead><tbody>${rows.map((row) => `<tr><td>${escapeHtml(dateText(row.createdAt))}</td><td>${escapeHtml(row.transactionType)}</td><td>${escapeHtml(row.billNumber || row.referenceNumber || '—')}</td><td>₹${Math.round(row.paidAmount)}</td><td>₹${Math.round(row.promotionalAmount)}</td><td>₹${Math.round(row.amount)}</td><td>₹${Math.round(row.newPaidBalance + row.newPromotionalBalance)}</td></tr>`).join('')}</tbody></table></body></html>`); popup.document.close(); popup.print();
  };

  const printReceipt = () => {
    if (!receipt) return; const { wallet, transaction } = receipt;
    const popup = window.open('', '_blank', 'width=520,height=720'); if (!popup) return;
    popup.document.write(`<html><head><title>Wallet Receipt</title><style>@page{size:auto;margin:6mm}@media print{html,body{height:auto !important}}body{font-family:Arial;padding:16px}.line{display:flex;justify-content:space-between;border-bottom:1px dashed #bbb;padding:9px 0}h2,p{text-align:center}small{color:#555}</style></head><body><h2>Cafe Aadvikam</h2><p>Wallet Credit Receipt</p><div class="line"><span>Transaction</span><b>${escapeHtml(transaction.id.slice(0, 8).toUpperCase())}</b></div><div class="line"><span>Customer</span><b>${escapeHtml(wallet.customerName)}</b></div><div class="line"><span>Wallet</span><b>${escapeHtml(wallet.walletNumber)}</b></div><div class="line"><span>Previous balance</span><b>₹${Math.round(transaction.previousPaidBalance + transaction.previousPromotionalBalance)}</b></div><div class="line"><span>Paid credit</span><b>₹${Math.round(transaction.paidAmount)}</b></div><div class="line"><span>Promotional credit</span><b>₹${Math.round(transaction.promotionalAmount)}</b></div><div class="line"><span>New balance</span><b>₹${Math.round(transaction.newPaidBalance + transaction.newPromotionalBalance)}</b></div><p><small>${escapeHtml(dateText(transaction.createdAt))}</small></p></body></html>`); popup.document.close(); popup.print();
  };
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="grid gap-2 border-b border-slate-100 p-4 md:grid-cols-4 xl:grid-cols-7">
      ...
      <div className="max-h-[58vh] overflow-auto">
        <table className="w-full min-w-[900px] text-left text-sm"><thead className="sticky top-0 bg-slate-50 text-[10px] font-black uppercase tracking-wider text-slate-500"><tr>{['Customer', 'Wallet ID', 'Paid', 'Promotional', 'Available', 'Status', 'Actions'].map((heading) => <th key={heading} className="px-4 py-3">{heading}</th>)}</tr></thead>
          <tbody className="divide-y divide-slate-100">{filtered.map((wallet) => <tr key={wallet.id} className="hover:bg-slate-50/70"><td className="px-4 py-3"><p className="font-black text-slate-900">{wallet.customerName}</p><p className="text-xs text-slate-500">{wallet.mobile}</p></td><td className="px-4 py-3 font-mono text-xs font-bold">{wallet.walletNumber}</td><td className="px-4 py-3 font-bold">{formatCurrency(wallet.paidBalance)}</td><td className="px-4 py-3 font-bold text-purple-700">{formatCurrency(wallet.promotionalBalance)}</td><td className="px-4 py-3 font-black text-emerald-700">{formatCurrency(wallet.totalBalance)}</td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-[10px] font-black uppercase ${wallet.status === 'active' ? 'bg-emerald-100 text-emerald-800' : wallet.status === 'suspended' ? 'bg-amber-100 text-amber-800' : 'bg-slate-200 text-slate-700'}`}>{wallet.status}</span></td><td className="px-4 py-3"><div className="flex gap-1"><button onClick={() => setProfile(wallet)} className="rounded-lg border p-2" title="View profile"><Eye className="size-3.5" /></button><button onClick={() => openCredit(wallet)} disabled={wallet.status === 'closed'} className="rounded-lg bg-emerald-600 p-2 text-white disabled:opacity-40" title="Add money"><Plus className="size-3.5" /></button><button onClick={() => openLimits(wallet)} disabled={wallet.status === 'closed'} className="rounded-lg border p-2 disabled:opacity-40" title="Limits"><Settings2 className="size-3.5" /></button></div></td></tr>)}</tbody>
        </table>
