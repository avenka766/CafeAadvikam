...
check(
  'Historical branch data uses paged dedicated loaders',
  branchOpsStore.includes('SPARSE_OPERATION_HISTORY_TYPES')
    && branchOpsStore.includes('loadSparseOperationHistory')
    && branchOpsStore.includes('["purchasePayments", "supplier_payment"]')
    && branchOpsStore.includes('["salespeople", "salesperson"]')
    && branchOpsStore.includes('["auditLogs", "audit_log"]')
    && snbAdminReports.includes('fetchSnbOperationHistory')
    && snbAdminReports.includes('operationBills')
    && snbAdminReports.includes('operationReturns')
    && snbAdminReports.includes('.range(from, from + pageSize - 1)')
    && adminSnb.includes('dbReports.operationBills')
    && adminSnb.includes('dbReports.operationReturns')
    && branchBusinessModules.includes(".from('branch_bill_headers')")
