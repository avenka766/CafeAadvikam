-- Restore the SNB-only flexible-stock rule without replacing the current
-- checkout implementation. The wrapper temporarily supplies only the missing
-- quantity for exact Mix & Combo category items, then delegates to the latest
-- available atomic checkout function. The delegated checkout deducts the sold
-- quantity, so these items finish at zero stock rather than going negative.
-- VRSNB and every other category remain under strict stock validation.

create or replace function public.complete_branch_checkout_canonical_v4(
  p_branch text,
  p_items jsonb,
  p_payments jsonb,
...
declare
  ...
  v_category text;
begin
  if p_branch not in ('SNB', 'VRSNB', 'Hosur') then
  ...
  if p_branch = 'SNB' then
    ...
    loop
      ...
      if lower(regexp_replace(trim(coalesce(v_category, '')), '\s+', ' ', 'g'))
        ...
        where branch = 'SNB'
          and item_barcode = v_barcode
        limit 1;

        v_available := coalesce(v_available, 0);
        if v_available < v_quantity then
          perform public.increment_branch_stock_by_barcode(
            'SNB',
            v_barcode,
  ...
  begin
    execute $call$
      select to_jsonb(public.complete_branch_checkout_canonical_v3(
        p_branch => $1,
        p_items => $2,
        p_payments => $3,
        p_customer_name => $4,
...
revoke all on function public.complete_branch_checkout_canonical_v4(
  text, jsonb, jsonb, text, text, text, text, numeric, numeric, numeric,
