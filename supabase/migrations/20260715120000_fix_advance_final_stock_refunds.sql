-- Finalize advance orders against the exact dispatched item first, and support
-- overpayment refunds and additional charges in one atomic transaction.
...
begin
  ...
  select coalesce(sum(amount) filter (where payment_stage in ('advance','balance')),0),
         max(advance_order_id)
    into paid, advance_order_id_value
...
revoke all on function public.finalize_branch_advance_order_v2(text,text,jsonb,numeric,numeric,text,text,text,boolean,numeric,numeric,numeric,text) from public;
grant execute on function public.finalize_branch_advance_order_v2(text,text,jsonb,numeric,numeric,text,text,text,boolean,numeric,numeric,numeric,text) to anon, authenticated, service_role;
